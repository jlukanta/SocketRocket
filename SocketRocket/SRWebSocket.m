...
- (void)didConnect
{
    ...
    if (SecRandomCopyBytes(kSecRandomDefault, keyBytes.length, keyBytes.mutableBytes) == -1) {
        [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain code:2134 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid SecRandomCopyBytes return value"] forKey:NSLocalizedDescriptionKey]]];
    }
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data;
{
    ...
    if (!useMask) {
        ...
        if (SecRandomCopyBytes(kSecRandomDefault, sizeof(uint32_t), (uint8_t *)mask_key) == -1) {
            [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain code:2134 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid SecRandomCopyBytes return value"] forKey:NSLocalizedDescriptionKey]]];
        }
